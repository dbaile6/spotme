...
import AudioFeaturesChart from './AudioFeatChart.js'
import FeatureGuide from './FeatureGuide.js'
import Search from './search.js'
import WeeklyList from './WeeklyList.js'

class SpotifyFeats extends React.Component {
  constructor(props) {
    super(props)
    this.state = {
  ...
  onTracksFetched(items) {
    ...
    if (trackIDs.length < 1) {
      this.setState({ message: 'No tracks available' })
      return
    }
  ...
  onAudioFeaturesError(error) {
    console.error('Cannot load audio characteristics', error)
    if (error.response.status === 401) {
      this.unauthorized()
    }
  }
  ...
  title() {
    if (this.state.activeView === 'search') {
      return 'Create a personalized playlist utilizing specific characteristics'
    }
